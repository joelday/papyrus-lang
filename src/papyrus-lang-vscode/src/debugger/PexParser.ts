...
export class PexReader {
    ...
    private readonly StringTableParser = new Parser().uint16('__tbllen').array('__strings', {
        type: this.StringParser(),
        formatter: (x: any): string[] => x.map((y: any) => y.__string),
        length: '__tbllen',
    });

    private readonly _strTableNest = {
        type: this.StringTableParser,
        formatter: (x: any): PexStringTable => {
    ...
    private readonly FunctionInfoRawParser = () =>
        new Parser()
            .uint16('ObjectName')
            .uint16('StateName')
            .uint16('FunctionName')
    ...
    private readonly FunctionInfosParser = () =>
        new Parser().uint16('__FIlen').array('__infos', {
            type: this.FunctionInfoRawParser(),
            length: '__FIlen',
            formatter: (x: any): FunctionInfo[] =>
                x.map((y: any) => {
                    const functinfo = {
    ...
    private readonly PropertyGroupRawParser = () =>
        new Parser()
            .uint16('ObjectName')
            .uint16('GroupName')
            .uint16('DocString')
    ...
    private readonly PropertyGroupsParser = () =>
        new Parser().uint16('__PGlen').array('__infos', {
            type: this.PropertyGroupRawParser(),
            length: '__PGlen',
            formatter: (x: any): PropertyGroup[] =>
                x.map((y: any) => {
                    const pgroups = {
                        ObjectName: this.TableLookup(y.ObjectName),
    ...
    private readonly StructOrderRawParser = () =>
        new Parser()
            .uint16('StructName')
            .uint16('OrderName')
            .uint16('NamesCount')
    ...
    private readonly StructOrdersParser = () =>
        new Parser().uint16('__SOlen').array('__infos', {
            type: this.StructOrderRawParser(),
            length: '__SOlen',
            formatter: (x: any): StructOrder[] =>
                x.map((y: any) => {
                    const sorders = {
    ...
    private readonly _doParseDebugInfo = () => {
        return new Parser()
            ...
            .choice('StructOrders', {
                tag: () => {
                    const val = this.endianness === 'little' ? 1 : 0;
                    return val;
                },
                choices: {
                    0: new Parser().skip(0),
    ...
    private readonly ParseDebugInfo = () =>
        new Parser().uint8('HasDebugInfo').choice('DebugInfo', {
            tag: 'HasDebugInfo',
            choices: {
                0: new Parser().skip(0),
    ...
    private readonly _debugInfoNest = {
        type: this.ParseDebugInfo(),
        formatter: (x: any): DebugInfo | undefined => (x ? x.DebugInfo : undefined),
    };
    ...
    private ReadPexBinary(buffer: Buffer): PexBinary | undefined {
        const endianness: 'little' | 'big' | undefined = DetermineEndianness(buffer);
        if (!endianness) {
            return undefined;
        }
