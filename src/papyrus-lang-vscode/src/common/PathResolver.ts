...
export interface IPathResolver {
    ...
    getLanguageToolPath(): Promise<string>;
    getDebugToolPath(): Promise<string>;
...
export class PathResolver implements IPathResolver {
    ...
    public async getLanguageToolPath(): Promise<string> {
        return this._asExtensionAbsolutePath('./bin/Debug/netcoreapp3.0/win-x64/DarkId.Papyrus.Server.Host.exe');
    }

    public async getDebugToolPath(): Promise<string> {
        return this._asExtensionAbsolutePath(
            './debug-bin/Debug/netcoreapp3.0/win-x64/DarkId.Papyrus.DebugAdapterProxy.exe'
        );
    }
    ...
    public async getInstallPath(game: PapyrusGame): Promise<string> {
        return resolveInstallPath(game, (await this._getGameConfig(game)).installPath);
    }
    ...
    public async getDebugPluginInstallPath(game: PapyrusGame, legacy?: boolean): Promise<string> {
        ...
        if (config.modDirectoryPath) {
            return path.join(
                await this.getModDirectoryPath(game),
                'Papyrus Debug Extension',
                this._getModMgrExtenderPluginPath(game),
                getPluginDllName(game, legacy)
            );
        } else {
            return path.join(
                await this.getInstallPath(game),
                this._getExtenderPluginPath(game),
                getPluginDllName(game, legacy)
            );
        }
    }
