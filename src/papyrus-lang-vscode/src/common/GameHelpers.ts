...
export async function FindUserGamePath(game: PapyrusGame, variant: GameVariant): Promise<string | null> {
  let GameFolderName: string = GetUserGameFolderName(game, variant);
  let home = getHomeFolder();
  if (!home) {
      return null;
  }
  let userGamePath = path.join(home, 'Documents', 'My Games', GameFolderName);
...
async function findSkyrimSEEpic(): Promise<string | null> {
  ...
  let epicAppdatapath = await getRegistryValueData(key, val);
  ...
  if (await exists(manifestsDir)) {
      ...
      for (const manifestFile of manifestFiles) {
          ...
          let data = await readFile(path.join(manifestsDir, manifestFile), 'utf8');
          if (data) {
...
async function findSkyrimSEGOG(): Promise<string | null> {
  ...
  for (const key of keynames) {
      let gogpath = await getRegistryValueData(key, 'path');
